...
export const SignUp = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Check if user session was immediately established, or if they need to check email
      if (data?.session) {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-950 px-4 py-12 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="absolute top-1/4 right-1/4 translate-x-1/2 -translate-y-1/2 w-80 h-80 rounded-full bg-violet-600/10 blur-[100px] pointer-events-none" />
      <div className="absolute bottom-1/4 left-1/4 -translate-x-1/2 translate-y-1/2 w-80 h-80 rounded-full bg-indigo-600/10 blur-[100px] pointer-events-none" />
      ...
      <div className="w-full max-w-md space-y-8 p-8 sm:p-10 bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl relative z-10">
        ...
        <form className="mt-8 space-y-5" onSubmit={handleSubmit}>
          ...
          {successMsg && (
            <div className="p-3.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-xs text-emerald-450 font-medium">
              {successMsg}
            </div>
          )}
